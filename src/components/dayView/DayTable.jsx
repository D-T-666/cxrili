    ...
    return data.tables.map((table, dayIndex) => {
      ...
      blocks = blocks.map((block, index) => {
        ...
        return {
          ...
          id:
            block.name + "s" + block.start + "e" + block.end + "d" + table.name,
          upVoters: [],
          downVoters: [],
          notes: [],
          classIndex: index,
                ...
                blockNotes.push({
                  ...
                  createdAt:
                    note.createdAt !== null
                      ...
                      : timeNow,
                  authorName: users[note.author]
                    ? users[note.author].name
                    : "unknown user",
                  authorPhotoURL: users[note.author]
                    ? users[note.author].photoURL
                    : ""
                })
            })
  ...
  return (
    <div className='content-box'>
      <ul className='time-table-timeline'>
        {tables.length > 0 &&
          tables[props.day].map((block, blockIndex) => (
            <ClassBlock
              key={block.id}
