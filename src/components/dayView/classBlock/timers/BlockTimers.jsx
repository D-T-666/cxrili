...
const ProgressBar = ({ percentage, active, timeLeft }) => {
  ...
  return (
    <ul className='progress-bar' style={{ opacity: active ? 1 : 0.2 }}>
      <div className='left' style={{ flexGrow: percentage }}>
        {" "}
      </div>
  ...
  tick() {
    ...
    if (this.props.shouldUpdate) {
      ...
      if (currentTime >= this.props.classData.int_start) {
        percentage =
          (getTimeInSeconds() / 60 - this.props.classData.int_start) /
          (this.props.classData.int_end - this.props.classData.int_start)

        if (percentage >= 1) percentage = 1
        if (!this.props.shouldUpdate) percentage = 0
    ...
    return (
      <div className='block-timers'>
        {this.props.showStartAndFinish ? (
          <ul>
            <li className='block-timer start'>
              {" "}
              {this.props.classData.start}{" "}
            </li>
            {this.props.expanded ? (
              <ProgressBar
